...
public class AlmostConstantAscertainedBeerLikelihoodCore extends LikelihoodCore {
    ...
    private void updateStateState(int state1, int state2, int offset, double[] matrices1,
            double[] matrices2, double[] partials3) {
    ...
    private void updateStatePartials(int state1, int offset, int partial2offset, double[] matrices1,
                double [] partials2, double[] matrices2, double[] partials3) {

        double sum, tmp;

        int u = offset;
        int v = partial2offset;
...
//            }
	...
	//	            for (int k = 0; k < nrOfPatterns; k++) {
		                ...
		                for (int i = 0; i < nrOfStates; i++) {
		                    ...
		                    for (int j = 0; j < nrOfStates; j++) {
		                        sum1 += matrices1[w] * partials1[v1 + j];
		                        sum2 += matrices2[w] * partials2[v2 + j];
